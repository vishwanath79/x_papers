...
def get_papers_list():
    ...
    try:
        ...
        if not new_papers:
            return "<div class='container'><h2>No papers found for today or error occurred. Please try again later.</h2></div>"
        ...
        if all_papers:
            ...
            for paper in all_papers:
                html_content += f"""
                    <tr>
                        <td>{paper['title']}</td>
                        <td>{paper.get('date_added', yesterday)}</td>
        ...
        error_message = f"<div class='container'><h2>An error occurred: {str(e)}</h2></div>"
        print(f"Error in get_papers_list: {str(e)}")
